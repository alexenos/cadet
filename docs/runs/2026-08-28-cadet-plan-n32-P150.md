...
- [Metric history — thinned from 1,954 rollouts](assets/2026-08-28-cadet-plan-n32-P150-history.csv)
...
**PPO's own diagnostics confirm the taper is too aggressive.** `approx_kl` measures how far
each update moves the policy; PPO normally targets ~0.01:

| window | approx_kl max | mean | clip_fraction mean |
|---|---|---|---|
| pre-taper | 0.056 | 0.0028 | 0.068 |
| **during taper** | **0.342** | 0.0066 | **0.138** |
| after taper | 0.014 | 0.0011 | 0.027 |

A KL of 0.34 is roughly 30× the usual target — the policy is being yanked, not nudged, and
`clip_fraction` doubles because the trust region is constantly binding. This is a mechanism,
not just a correlation: the moving threshold changes the augmented reward faster than PPO's
trust region is sized to absorb, so each update fights the last. It is direct evidence that
the taper rate, rather than the constraint itself, causes the collapse.

### Efficiency rises monotonically even while reward collapses

Reward is a misleading progress signal here, because it says nothing about what the policy
paid. Targets won per unit of budget-normalised energy tells a different story:

| window | mean efficiency |
|---|---|
| pre-taper | 3.52 |
| **during the reward collapse** | **3.63** |
| post-taper | 23.31 |

Efficiency is *flat* through the window where reward falls 80%. The policy was not getting
worse during the squeeze — it was being pushed down the power axis while holding its
targets-per-joule roughly constant. It then improves ~6× once the constraint stops moving,
ending 26× above where it started.

If a single curve had to summarise learning progress in a constrained problem like this,
it should be this one rather than reward.

### Delegation is learned early, and not because of the budget

`n_delegate` reaches 295/300 by **99k** timesteps and explicit rolls fall below 1 by **200k**
— before the slack taper has meaningfully begun (it starts at 66k and only reaches 1 at
726k). The policy therefore does not delegate to save power; it delegates because the exact
SSP dynamic program is simply better at maneuvering than anything it can learn. Power
pressure arrives long afterwards and changes only the *sensing* schedule.
...
- **One seed, 20 evaluation episodes.** The paper's protocol uses more; run-to-run variance
  here is unmeasured.
- **The shortfall is real policy underperformance, not baseline miscalibration.** An
  earlier version of this write-up argued that scoring against the paper's baselines would
  lift 17.9% to ~31%. That rested on a back-solved SSP of 194, taken from the rounded
  averages in the abstract. Back-solving instead from the full 24-cell Table 2 grid gives
  SSP 209.5 and Oracle 291.5 — within 1% of this implementation's 211.2 and 290.2. The
  baselines are correct, so none of the gap is explained away. See *Baseline bounds* in the
  reproduction notes.
