...
> **2026-09-01.** The paper's first author [replied](author-correspondence.md) and named two
> conventions this implementation had guessed differently. They account for the whole of the
> capture shortfall recorded below, and they retire the Table 3 "typo" claim. Sections
> affected are marked; the finding is written up in
...
> **Resolved 2026-09-01, and the conclusion of this section is wrong.** The shortfall was
> two modelling conventions, not a weaker policy. Re-scoring these same checkpoints under
> the author's conventions gives 267.6 and 264.1 targets — 69.3% and 65.2% of the gap
> closed, above the paper's 56.1% and 60.1%. See
> [`shortfall-resolved.md`](shortfall-resolved.md).
>
> The section is kept in full because the eliminations below remain valid negative results,
> and because they are why the modelling difference was findable: once baselines, σ_A, the
> constraint machinery, coverage, sensing volume and hyperparameters had each been ruled out
> by direct test, what was left had to be something the paper did not state.
...
### Retracted: Table 3's P̄ = 1500 accuracy is not a typo

This section previously argued that Table 3's 0.202 could not be reconciled: under
clear-captures ÷ targets-encountered, 258.8 captures implies 1,281 encounters against a
provable DP bound of ~607, so the row had to be inconsistent. **That was wrong.**

The paper states the definition in *Metrics* — "Fraction of payload captures that
successfully image cloud-free targets" — and the author confirmed it: the denominator is
capture *actions*, and a policy with surplus power fires on epochs where its footprint is
empty. The arithmetic was correct on the wrong denominator.

Re-scoring the committed 20-episode evaluations with only the denominator changed:
...
Capture accuracy at P̄ = 1500 *is* a usable comparison target, and this implementation
already matches it to within 2%. Correcting the denominator in `env.statistics()` is change
2 in [`shortfall-resolved.md`](shortfall-resolved.md) §4.

One piece is still open: under this denominator the `ssp` baseline, which fires every epoch,
scores 0.070 rather than the 35% quoted beside Figure 5 — 35% being the cloud-free base
rate, which the ratio returns only if the baseline is charged for scheduled captures alone.
That question is in the [correspondence](author-correspondence.md).

## What is not reproduced

> The two trained cells below are reported under this repo's own conventions. Under the
> author's, the same weights score 69.3% and 65.2% — see
> [`shortfall-resolved.md`](shortfall-resolved.md). Neither set of numbers is a
> reproduction: two cells of twenty-four, one seed each.
